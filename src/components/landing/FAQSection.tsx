...
const faqs = [
  ...
  },
  {
    ...
    a: "Sim. Os áudios das consultas são descartados automaticamente, os dados são anonimizados de acordo com a LGPD e nenhuma informação dos pacientes circula fora da MindMed.",
  },
  {
    q: "A MindMed substitui o julgamento médico?",
    a: "Não. A MindMed é uma ferramenta de apoio à documentação. Toda informação gerada passa pela revisão e aprovação do profissional.",
  },
];
...
const FAQSection = () => {
  ...
  return (
    <section className="py-16 md:py-32">
      <div className="container mx-auto max-w-3xl px-5 md:px-6">
        <FadeIn>
          ...
          <h2 className="font-mono text-2xl md:text-[2.5rem] font-bold tracking-tight text-foreground text-center leading-tight">
            O que os médicos{" "}
            ...
            <FadeIn key={i} delay={i * 0.04}>
              <div className="rounded-xl border border-border bg-background overflow-hidden transition-shadow hover:shadow-sm">
                <button
                  ...
                  className="flex items-center justify-between w-full text-left px-4 md:px-6 py-4 md:py-5"
                >
                  <span className="text-sm md:text-[15px] font-semibold text-foreground pr-4 md:pr-6">{faq.q}</span>
                  <div className="h-6 w-6 md:h-7 md:w-7 rounded-full bg-primary/10 flex items-center justify-center shrink-0">
                    {openIndex === i ? (
                      <Minus className="h-3 w-3 md:h-3.5 md:w-3.5 text-primary" />
                    ) : (
